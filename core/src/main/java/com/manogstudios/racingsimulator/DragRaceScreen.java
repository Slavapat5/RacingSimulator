...
public class DragRaceScreen implements Screen {
    ...
    public void show() {
        ...
        Label versusLabel = new Label("YOU vs AI", skin);
...
// Toggle show/hide on main button click
    ...
    private void showFinishDialog(long timeMillis, boolean playerWon) {
        ...
        summaryTable.add(winnerLabel).padBottom(30).row();
        ...
        Label resultLabel = new Label(playerWon ? "You Win!" : "You Lose!", skin);
        resultLabel.setFontScale(4f);
        resultLabel.setAlignment(Align.center);
        resultLabel.setColor(playerWon ? Color.GREEN : Color.RED);
...
// Only update AI if race is not finished
        ...
        if (!raceFinished && finishZone != null) {
            Car aiCar = cars.get(1);

            boolean playerFinished = finishZone.overlaps(playerCar.getBoundingRectangle());
            boolean aiFinished = finishZone.overlaps(aiCar.getBoundingRectangle());
